...
const bracketLevels = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh'] as const;
...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  description: 'A JupyterLab extension adding rainbow brackets/parentheses to the editor.',
  autoStart: true,
  requires: [IEditorExtensionRegistry],
  optional: [ITranslator],
  activate: (app: JupyterFrontEnd, extensions: IEditorExtensionRegistry, translator: ITranslator | null) => {
    const trans = (translator ?? nullTranslator).load('jupyterlab-rainbow-brackets');
    ...
    extensions.addExtension(
      Object.freeze({
        ...
        factory: () =>
          EditorExtensionRegistry.createConfigurableExtension((settings: IRainbowSettings) => {
            const styles: Record<string, {color: string}> = {}
            for (const level of bracketLevels) {
              const rainbowClass = '.rainbow-bracket-' + RAINBOW_CLASSES[level]
              styles[`${lightSelector} ${rainbowClass}`] = { color: settings.light[level] }
              styles[`${lightSelector} ${rainbowClass} > span`] = { color: settings.light[level] }
              styles[`${darkSelector} ${rainbowClass}`] = { color: settings.dark[level] }
              styles[`${darkSelector} ${rainbowClass} > span`] = { color: settings.dark[level] }
            }
